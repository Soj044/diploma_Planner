...
### Update Note (2026-05-11)
- Local CPU-first runtime defaults were narrowed for more stable development on mid-range machines:
  - `OLLAMA_CHAT_MODEL` now defaults to `llama3.2:3b`
  - `OLLAMA_EMBED_MODEL` stays `bge-m3`
  - `OLLAMA_TIMEOUT_SECONDS=90`
- Explanation prompts were compacted to task/proposal/diagnostic summaries plus short similar-case excerpts instead of full raw JSON dumps, to reduce local `ollama /api/chat` pressure while preserving the public response contract.

## ADR-021: AI Explanations Use Hybrid Planner Comparison Facts Plus Live Core Availability Context

- Date: 2026-05-12
- Status: accepted

### Context
После первого end-to-end AI smoke выяснилось, что explanation payloads были слишком общими:
- `ai-layer` видел только выбранного сотрудника и общие `eligibility_count`/`score_map`;
- `planner-service` не сохранял structured reasons по невыбранным кандидатам;
- `core-service` не отдавал live comparison context по альтернативным сотрудникам;
- локальная LLM не могла надежно объяснить, почему не выбрали другого сотрудника, даже если у него был leave overlap или ниже persisted score.

### Decision
- Добавить в `planner-service` persisted `candidate_analysis` внутри `PlanRunArtifacts`.
- Генерировать `candidate_analysis` из hard-filter traces + scores + selected proposals после optimizer stage.
- Использовать стабильные `outcome_code` значения для AI reasoning:
  - `selected`
...
- Расширить planner internal AI contexts (`proposal-context`, `unassigned-context`) полем `candidate_analysis`.
- Расширить core internal `assignment-context` query param `comparison_employee_ids` и response `comparison_employees[]` + `availability_facts`.
- Оставить public `ai-layer` response shape без изменений, но сделать generation hybrid:
  - LLM по-прежнему возвращает `summary/reasons/risks/recommended_actions/advisory_note`
  - `ai-layer` детерминированно добавляет comparison reasons из `candidate_analysis` и live availability facts, даже если LLM отвечает слишком общо.

### Consequences
- Плюсы: explanation quality перестает зависеть только от prompt engineering и начинает опираться на backend-owned comparison facts.
- Плюсы: AI теперь может явно объяснять различие между “ниже score”, “не хватило availability”, “leave overlap”, и “missing skill”.
- Плюсы: фронтенд не требует новых API полей и продолжает использовать прежний advisory payload.
- Минусы: planner artifacts стали богаче, а internal AI contexts — шире; это увеличивает поддержку тестов и persistence compatibility.

## ADR-020: Date-Based Task Eligibility Uses Cumulative Availability Hours

- Date: 2026-05-11
- Status: accepted

### Context
The first AI-assisted `/tasks/new` smoke uncovered a planner mismatch for date-based multi-day tasks. `core-service` exports daily availability slots, but the initial planner MVP required one continuous slot to cover the whole task interval. This caused obviously eligible employees to be filtered out and forced `/tasks/new` directly into manual assignment mode even when the employee had enough total hours across the task window.

### Decision
- Keep the existing hard filters for:
  - active employee
  - department match
  - required skill thresholds
- Change the availability rule in `planner-service`:
  - collect every availability slot that intersects the task window
  - sum `available_hours` across those slots
  - if `available_hours` is missing on a slot, derive hours from the overlapping slot duration
  - treat the employee as eligible when cumulative hours in the task window are greater than or equal to `task.estimated_hours` (or the normalized task duration when `estimated_hours` is absent)
- Keep snapshot format unchanged; `core-service` continues exporting day-level availability slots.
