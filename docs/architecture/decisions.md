...
## ADR-015: Stage 1 Role Contracts Add Employee Assignment Reads and Manual Final Assignment Path

- Date: 2026-04-30
- Status: accepted

### Context
После завершения frontend Milestone 1 backend всё ещё не покрывал два новых продуктовых сценария:
- employee-facing список собственных final assignments;
- explicit manager/admin manual assignment path без planner proposal approval.

Параллельно employee self-service rules для schedules и leaves оставались слишком широкими для нового role-based UX.

### Decision
- Расширить auth payloads (`signup`, `login`, `refresh`, `me`) richer `employee_profile` полями `hire_date` и `is_active`.
- Расширить `GET /api/v1/departments/` nested employee summaries без email leakage.
...
- Сохранить общий invariant для planner approval и manual assignment:
  у одной задачи не может появиться второй non-rejected final assignment.

### Consequences
- Плюсы: backend contracts лучше совпадают с новым role-aware product flow, final assignment остаётся backend-owned truth в `core-service`, а manual path не превращается в неконтролируемый generic CRUD.
- Минусы: Stage 1 backend теперь опережает текущий employee frontend UX, поэтому до следующего frontend slice ожидаемы интеграционные расхождения на schedule/leave экранах.
